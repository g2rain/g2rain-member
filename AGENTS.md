...
- Profile：`java-domain-service`
- 当前正式版本：`1.0.0`
- 中央仓库：`https://github.com/g2rain/g2rain`
- 固定版本：`architecture-v1.0.0`
- Profile 路径：`docs/architecture/profiles/java-domain-service`
- 本项目例外：`docs/architecture/deviations.md`

中央 Profile 是同类项目公共规则的事实来源；本项目 docs 维护 Member 领域职责、当前落地说明和例外。Agent 必须读取项目声明的固定 Tag，不能改为跟随中央仓库 `main` 或开发分支。
...
1. `docs/project.yaml`
2. 中央 `java-domain-service` Profile
3. `docs/architecture/deviations.md`
4. `docs/architecture/overview.md`
5. `docs/architecture/modules.md`
...
10. 当前需求对应的 `docs/requirements`、`docs/design` 或 `docs/decisions` 文档
...
- 仅有效记录唯一且逻辑删除后允许重建时，必须使用 `(IF(delete_flag = 0, 0, NULL))` 函数唯一索引；不得使用普通 `UNIQUE (..., delete_flag)`。永久占位的业务键则不在唯一索引中加入 `delete_flag`。
...
- 不能用本地规则静默覆盖中央 Profile；有意偏离时更新 `docs/architecture/deviations.md`，长期跨项目变化提交中央 ADR。
