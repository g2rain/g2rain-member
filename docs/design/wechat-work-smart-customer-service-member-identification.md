...
数据库和两张会员表统一使用 MySQL 8 `utf8mb4_0900_as_cs` 排序规则。会员编号、身份类型、企业微信 `external_userid` 等字符串的比较均大小写敏感；调用方不得依赖数据库进行大小写归一化。
...
身份唯一索引不包含 `delete_flag` 是有意设计：已删除身份继续占用其租户内唯一值，防止外部身份在没有核验的情况下被自动转移到另一个会员。
